...
BAKKESMOD_PLUGIN(JumpInReplay, "JumpInReplay", "0.1", PERMISSION_ALL)
...
void JumpInReplay::CvarRegister() {
	...
	cvarManager->registerCvar("jumpIn_test","0","test cvar for debuging",true,true,-100.0f,true,100.0f,false).addOnValueChanged(std::bind(&JumpInReplay::Test, this, std::placeholders::_1, std::placeholders::_2));
...
void JumpInReplay::TestHook(std::string eventName) {
	Log(std::to_string(gameWrapper->GetGameEventAsReplay().GetCars().Count()) + " Cars");
}
...
void JumpInReplay::StartPrivateMatch(std::string oldValue, CVarWrapper cvar) {
	if (cvar.getBoolValue() == true) {
		if (hasReplaySaved == true) {
			//disable normal color in freeplay
			hasColorsNormalized = cvarManager->getCvar("cl_freeplay_carcolor").getIntValue();
